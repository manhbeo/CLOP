...
def train(epochs, batch_size, dataset, pretrain_dir = None, OAR=True, loss="nxt_ent", devices=1, k=100, num_workers=9, 
    ...
    wandb_logger = pl.loggers.WandbLogger(project="CLOA_Train", name=f'{dataset}-{batch_size*devices}-{loss}-oar={OAR}-lr={lr}')
    ...
    trainer.save_checkpoint(f'{batch_size*devices}-{loss}-oar={OAR}-lr={lr}.ckpt')
