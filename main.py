...
def train(epochs, batch_size, dataset, pretrain_dir = None, OAR=True, supervised=False, devices=1, k=100, num_workers=9,
    ...
    wandb_logger = pl.loggers.WandbLogger(project="CLOA_Train", name=f'{dataset}-{batch_size*devices}-oar={OAR}-scale={scale}-gauss={gauss}')
    ...
    trainer.save_checkpoint(f'{batch_size*devices}-oar={OAR}-scale={scale}-gauss={gauss}.ckpt')
