...
class LinearClassifier(LightningModule):
    ...
    def configure_optimizers(
        ...
        optimizer = LARS(self.parameters(), 
                         lr=0.3 * self.batch_size_per_device * self.trainer.world_size / 256 if self.lr is not None else self.lr)
        scheduler = {
    ...
    def test_step(self, batch: Tuple[Tensor, ...], batch_idx: int) -> Tensor:
        ...
        Args:
            batch:
                A batch of data containing images and corresponding targets.
            batch_idx:
                Index of the batch.
