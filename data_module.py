...
class CustomDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, drop_last=True, num_workers=24)
...
class CustomEvaluationDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        if self.dataset == "cifar10":
            self.train_dataset = datasets.CIFAR10(self.data_dir, train=True, transform=self.transform, download=True)
            self.val_dataset = datasets.CIFAR10(self.data_dir, train=False, transform=self.transform, download=True)
        elif self.dataset == "cifar100":
            self.train_dataset = datasets.CIFAR100(self.data_dir, train=True, transform=self.transform, download=True)
            self.val_dataset = datasets.CIFAR100(self.data_dir, train=False, transform=self.transform, download=True)
        elif self.dataset == "imagenet":
